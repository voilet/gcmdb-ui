...
export default class Addcabinet extends PureComponent {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    // clean state
    //console.log(nextProps.gproline)
    //console.log(nextProps.gidc.bays.data == undefined || nextProps.gidc.bays.data == null  ? [] : nextProps.gidc.bays.data)
    if(nextProps.user){
      this.setState({
    ...
    if(nextProps.gidc.idc){
      console.log(nextProps.gidc)
      console.log(nextProps.gidc.cabinet.data == null ? [] : nextProps.gidc.cabinet.data)
      //console.log(nextProps.gidc.bays.data == undefined || nextProps.gidc.bays.data == null  ? [] : nextProps.gidc.bays.data)
      this.setState({
        idcData:nextProps.gidc.idc.data,
        cabinetData:nextProps.gidc.cabinet.data == null ? [] : nextProps.gidc.cabinet.data,
        baysData:nextProps.gidc.bays.data == undefined || nextProps.gidc.bays.data == null  ? [] : nextProps.gidc.bays.data
      })
    }
        ...
        if(!dateStatus){
          ...
          message.error('The two input passwords do not match!')
              ...
              const fields = {
                ...
                'start_guaratee':form.getFieldValue('start_guaratee') ? form.getFieldValue('start_guaratee') : "",
                'stop_guaratee':form.getFieldValue('stop_guaratee') ? form.getFieldValue('stop_guaratee') : "",
              ...
              for(let i=0;i<uuid;i++){
                project.push(form.getFieldValue(`probygid${i}`)? form.getFieldValue(`probygid${i}`) : "")
              }
  ...
  newDateChange(date, dateString){
    ...
    const oldDate = new Date(form.getFieldValue('stop_guaratee') ? form.getFieldValue('stop_guaratee') : "");
  ...
  oldDateChange(date, dateString){
    const { form }=this.props
    const newDate = new Date(form.getFieldValue('start_guaratee') ? form.getFieldValue('start_guaratee') : "");
    const dateVal = new Date(dateString)
  ...
  onprogroupbylidChange = (value) => {
    ...
    // this.setState({
    //   secondCity: value,
    //   homeData:homeobj[value],
    //   home:homeobj[value][0]
    // });
  }

  onprobygidChange = (value) =>{
    // this.setState({
    //   home: value,
    // });
  }
  ...
  render() {
    ...
    const { prolinedata, progroupbylid, probygid, idcData, hardwareData, cabinetData, baysData, userData } = this.state
    ...
    //console.log(probygid)
    //产品线列表
    const prolineOptions = prolinedata.map(proline => <Option key={proline.ID} value={proline.ID}>{proline.title}</Option>);
    const progroupOptions = progroupbylid.map(item => <Option key={item.ID} value={item.ID}>{item.title}</Option>);
    const probygidArr = probygid.map((obj)=>obj)
    const probygidOptions = probygidArr.map(probygidItem => <Option key={probygidItem.ID} value={probygidItem.ID}>{probygidItem.title}</Option>);
    //机架机房
    const idcOptions = idcData.map(item => <Option key={item.ID} value={item.ID}>{item.Title}</Option>);
    const cabinetOptions =cabinetData.length == undefined ? [] : cabinetData.map(item => <Option key={item.ID} value={item.ID}>{item.Title}</Option>);
    const baysOptions = baysData.map(item => <Option key={item.ID} value={item.ID}>{item.Title}</Option>);
        ...
        <Col span={7}>
            <FormItem>
              {getFieldDecorator(`proline${k}`,{
                //initialValue: prolinedata.length == 0 ? "" : prolinedata[0].title
                rules: [{
                  required: true,
          ...
          <Col span={8}>
            <FormItem>
              {getFieldDecorator(`progroupbylid${k}`,{
                //initialValue: progroupbylid.length == 0 ? "" : progroupbylid[0].title
                rules: [{
                  required: true,
          ...
          <Col span={7}>
            <FormItem>
              ...
              // initialValue:probygid.length == 0 ? "" : probygid[0].title
              rules: [{
                required: true,
                ...
                <Select 
                  showSearch
                  style={{ width: 100, marginRight: 30 }} 
                  onChange={this.onprobygidChange}
                  optionFilterProp="children"
                  filterOption={(input, option) => option.props.children.toLowerCase().indexOf(input.toLowerCase()) >= 0}
                >
                  {probygidOptions}
      ...
      <Row gutter={24}>    
        <Col span={12} key ={1} style={{ display: 'block' }}>
          ...
          {getFieldDecorator(`plan_id`, {
              ...
              >
                {
                  hardwareData==undefined?[]:hardwareData.map((obj)=>{
          ...
          <FormItem label="机器密码">
            {getFieldDecorator(`password`, {
                rules: [{
                  ...
                  <Input type="password" placeholder="请输入机器密码" onBlur={this.passwordBlur.bind(this)}/>
              )}
          </FormItem>
        </Col>
        <Col span={12} key ={2} style={{ display: 'block' }}>
          <FormItem label="确认机器密码">
            {getFieldDecorator(`surePwd`, {
                rules: [{
                  ...
                  <Input type="password" placeholder="请输入确认机器密码" onBlur={this.passwordBlur.bind(this)}/>
              )}
          </FormItem>
        </Col>
      </Row>
            ...
            <Col span={8}>
              <FormItem>
                {getFieldDecorator(`idc_id`,{
                  //initialValue: prolinedata[0].title
                  rules: [{
                    required: true,
