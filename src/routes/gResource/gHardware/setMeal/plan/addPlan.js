...
export default class Addplan extends PureComponent {
  ...
  render() {
    ...
    //const { cpu_plan_category, mem_plan_category, disk_plan_category, adator_plan_category, power_plan_category } = hardware
    ...
    const adator_plan_category = hardwaredata.data.adator_plan_category == undefined ? [] : hardwaredata.data.adator_plan_category
          ...
          >
            {getFieldDecorator('adaptor_id')(
              <Select
                ...
                placeholder="请选择电源套餐"
              >
                {adator_plan_category.map(item=>(<Option key={item.id} value={item.id}>{item.title}</Option>))}
              </Select>
            )}
          </FormItem>
